function varargout = drawContourAlone(command, varargin)
%"drawContourAlone"
...
%
%Usage:
%   To begin: drawContourAlone('axis', hAxis);
%   To quit : drawContourAlone('quit', hAxis);
%   Get Data: drawContourAlone('getContours', hAxis);
%   preDraw : drawContourAlone('setContours', hAxis, contour);
...
switch command
    ...
    case 'axis'
        ...
        set(hAxis, 'buttonDownFcn', 'drawContourAlone(''btnDownInAxis'')');
        set(hFig, 'WindowButtonUpFcn', 'drawContourAlone(''btnUp'')');
        set(hFig, 'doublebuffer', 'on');

    case 'quit'
        %Removed passed axis from drawContourAlone mode.
        hAxis = varargin{1};
        hFig  = get(hAxis, 'parent');
        noneMode(hAxis);
        setappdata(hAxis, 'contourV', []);
    ...
    case 'btnDownInAxis'
        ...
        set(hFig, 'WindowButtonMotionFcn', 'drawContourAlone(''motionInFigure'')');
...
if ~isempty(contourV)
    for i = 1:length(contourV)
        segment = contourV{i};
        if ~isempty(segment)
            hContour = [hContour, line(segment(:,1), segment(:,2), 'color', 'blue', 'linewidth', .5, 'hittest', 'off', 'erasemode', 'normal', 'userdata', i, 'ButtonDownFcn', 'drawContourAlone(''contourClicked'')', 'parent', hAxis)];
        end
    end
...
if ~isempty(segment) & strcmpi(mode, 'drawing')
    hSegment = line(segment(:,1), segment(:,2), 'color', 'red', 'hittest', 'off', 'erasemode', 'none', 'parent', hAxis, 'ButtonDownFcn', 'drawContourAlone(''contourClicked'')');
    setappdata(hAxis, 'hSegment', hSegment);
elseif ~isempty(segment)
    hSegment = line(segment(:,1), segment(:,2), 'color', 'red', 'hittest', 'on', 'erasemode', 'normal', 'parent', hAxis, 'ButtonDownFcn', 'drawContourAlone(''contourClicked'')');
    setappdata(hAxis, 'hSegment', hSegment);
else
