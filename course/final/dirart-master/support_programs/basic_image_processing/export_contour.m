function export_contour(filename,ctdim,ctinfo,zsinput)
%
% export_contour(filename,ctdim,ctinfo,zsinput)
%
...
while 1		% For each structure
	...
	if isfield(info.StructureSetROISequence,roi_item)
		ROIinfo = getfield(info.StructureSetROISequence,roi_item);
		ROIName = ROIinfo.ROIName;
		disp(sprintf('Processing contours for %s ...',ROIName));
		ROIContour = getfield(info.ROIContourSequence,roi_item);
...
%				disp(sprintf('For %s : contour %d',ROIName,c+1));
				item = getfield(ContourSequence,c_item);
...
%surfaces = flipdim(surfaces,3);
save surfaces.mat surfaces;
