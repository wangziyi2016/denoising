...
if( ~isempty(varargin) )
	...
	if length(dim) == 4
		if dim(3) == 3
			...
			error('Cannot support 4D image data');
		end
	end
	handles.dim = dim;
	...
	if( length(varargin) > 1 )
		for n = 2:length(varargin)
			...
			elseif ischar(varargin{n-1})
				...
				elseif prod(size(varargin{n})) == 3
					handles.aspects = varargin{n};
				elseif prod(size(varargin{n})) == 1
...
display_motion_field = ~isempty(handles.mvx);
