...
for i=1:size(phi,1)
	for k=1:size(phi,3)
		phi_x_minus(i+1,:,k+1) = der_ENO1_minus(data_ext(i+1,:,k+1), dx);
		phi_x_plus(i+1,:,k+1) = der_ENO1_plus(data_ext(i+1,:,k+1), dx);
		phi_x(i+1,:,k+1) = select_der_normal(Vn(i+1,:,k+1), phi_x_minus(i+1,:,k+1), phi_x_plus(i+1,:,k+1));
	end
end
...
for j=1:size(phi,2)
	for k=1:size(phi,3)
		phi_y_minus(:,j+1,k+1) = der_ENO1_minus(data_ext(:,j+1,k+1), dy);
		phi_y_plus(:,j+1,k+1) = der_ENO1_plus(data_ext(:,j+1,k+1), dy);
		phi_y(:,j+1,k+1) = select_der_normal(Vn(:,j+1,k+1), phi_y_minus(:,j+1,k+1), phi_y_plus(:,j+1,k+1));
	end
end
...
for i=1:size(phi,1)
	for j=1:size(phi,2)
		phi_z_minus(i+1,j+1,:) = der_ENO1_minus(data_ext(i+1,j+1,:), dz);
		phi_z_plus(i+1,j+1,:) = der_ENO1_plus(data_ext(i+1,j+1,:), dz);
		phi_z(i+1,j+1,:) = select_der_normal(Vn(i+1,j+1,:), phi_z_minus(i+1,j+1,:), phi_z_plus(i+1,j+1,:));
	end
end
