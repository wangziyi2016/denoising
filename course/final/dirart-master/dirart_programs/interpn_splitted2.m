function out = interpn_splitted2(y0,x0,z0,in,y,x,z,method,defval)
...
y = max(y,0); y = min(y,max(y0(:))+1);
x = max(x,0); x = min(x,max(x0(:))+1);
z = max(z,0); z = min(z,max(z0(:))+1);
...
for ntr = 1:nstr
	for nco = 1:nsco
		for nsa = 1:nssa
			...
			out(tr,co,sa) = interpn(y0,x0,z0,in,y(tr,co,sa),x(tr,co,sa),z(tr,co,sa),method,defval);
		end
	end
end
