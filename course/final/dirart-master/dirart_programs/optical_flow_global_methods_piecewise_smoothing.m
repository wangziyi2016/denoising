function varargout = optical_flow_global_methods_piecewise_smoothing(method,mainfigure,img1,imgt,voxelsizes,maxiter,stop,mvy0,mvx0,mvz0,offsets,masks,smoothing)
%
% Gloabl optical flow methods with piecewise smoothing
%
% Input parameters:
...
%				5:	0/1	- Reverse consistency (averging gradient of I1 and I2), on/off
% masks		-	the segmentation binary masks, each bit is a binary mask,
%				support multiple structure 
%
% Outputs:
...
if ~exist('smoothing','var') || isempty(smoothing)
	...
	smoothings = [0.05 0.1 0.2 0.4 0.8];
	smoothing = smoothings(smoothing);
% 	smoothing = 0.2;
end
...
if exist('masks','var') && ~isequal(size(masks),size(imgt))
	masks = masks(yoffs,xoffs,zoffs);
end
...
if options(1) == 0	% Original Horn-Schunck method
	% Parameters
	lambda = smoothing*max_intensity;
	fprintf('Lambda = %d\n',lambda);
	...
	lambda0 = smoothing*max_intensity;
...
% 		[Vya,Vxa,Vza]=hs_velocity_avg3d(Vy,Vx,Vz,masks);
		[Vya,Vxa,Vza]=hs_velocity_avg3d_weighted(Vy,Vx,Vz,5,masks,Iy,Ix,Iz);
	end
...
% 		ItIz2 = ItIz - lambda2*(Vyyz+Vxxz+Vzzz); clear Vyyz Vxxz Vzzz;
	...
	if options(2) == 1
% 		Vy = Vya - (Vya.*IyIy + Vxa.*IyIx + Vza.*IyIz + ItIy2) ./ suma;
% 		Vx = Vxa - (Vya.*IyIx + Vxa.*IxIx + Vza.*IxIz + ItIx2) ./ suma;
% 		Vz = Vza - (Vya.*IyIz + Vxa.*IxIz + Vza.*IzIz + ItIz2) ./ suma;
...
% 		beta = 0.05;
% 		Vy = Vya-(Iy.*sumb+ItIy2+beta*sign(Vy))./suma;
% 		Vx = Vxa-(Ix.*sumb+ItIx2+beta*sign(Vx))./suma;
% 		Vz = Vza-(Iz.*sumb+ItIz2)./suma;
		clear sumb;
	end
